"""Polymarket UpDown 1h exchange adapter.

继承 15m 实现，覆盖周期、slug 生成和对齐逻辑。
Slug 格式: bitcoin-up-or-down-february-16-8pm-et
"""
...
_MARKET_PERIOD_SECONDS = 60 * 60
_DEFAULT_MARKET_CLOSE_BUFFER = 300
...
class PolymarketUpDown1h(PolymarketUpDown15m):
    """Polymarket 1h UpDown market."""

    @classmethod
    def get_exchange_info(cls) -> Dict[str, str]:
        return {
            "id": "polymarket_updown1h",
            "name": "Polymarket UpDown 1h",
            "type": "prediction",
        }
    ...
    def _aligned_timestamp(self, offset: int = 0) -> int:
        """计算第 offset 个1小时周期的开始时间戳 (ET对齐)."""
        now = datetime.now(_ET_TZ)
        base = now.replace(minute=0, second=0, microsecond=0)
        return int((base + timedelta(hours=offset)).timestamp())

    def _build_slug(self, timestamp: int) -> str:
        """构建1h市场slug, 格式: bitcoin-up-or-down-february-16-8pm-et"""
        ...
        day = dt.day
        hour_str = dt.strftime("%I%p").lstrip("0").lower()
        return f"{market_name}-up-or-down-{month}-{day}-{hour_str}-et"
